...
function sendVideoStream() {
    ...
    // Optimize for low latency grid view (320x240 is sufficient for thumbnails)
    // AI detection uses separate high-res snapshots
    const videoWidth = video.videoWidth || 320;
    const videoHeight = video.videoHeight || 240;

    // Target 320x240 for display stream
    let canvasWidth = 320;
    ...
    // Low quality for fast transmission (0.5 is good for moving video)
